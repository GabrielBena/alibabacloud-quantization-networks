...
def get_data(
    split_id, data_dir, img_size, scale_size, batch_size, workers, train_list, val_list
):
    root = data_dir
    ...
    train_loader = DataLoader(
        Preprocessor(train_list, root=root, transform=train_transformer),
    ...
    val_loader = DataLoader(
        Preprocessor(val_list, root=root, transform=test_transformer),
        batch_size=batch_size,
        num_workers=workers,
        shuffle=False,
        pin_memory=True,
...
def main(args):
    ...
    def adjust_lr(epoch):
        ...
        lr = (
            args.lr
            if epoch < step_size
            else args.lr * (0.1 ** ((epoch - step_size) // decay_step + 1))
        )
...
class Trainer(object):
    ...
    def train(
        ...
        for i, inputs in enumerate(data_loader):
            ...
            alpha_grad, beta_grad = qua_op.updateQuaGradWeight(
                T, self.alpha, self.beta, init=self.init
            )
            for index in range(len(self.alpha)):
                self.alpha[index].grad = Variable(
                    torch.FloatTensor([alpha_grad[index]]).cuda()
                )
                self.beta[index].grad = Variable(
                    torch.FloatTensor([beta_grad[index]]).cuda()
                )
    ...
    def show_info(self, with_arch=False, with_grad=True):
        ...
        if with_grad:
            ...
            for name, param in self.model.named_parameters():
                ...
                print(
                    "{}: size{}, data_abd_avg: {}, dgrad_abd_avg: {}, data/grad: {}".format(
                        name,
                        param.size(),
                        mean_value,
                        mean_grad,
            ...
            for name, param in self.model.named_parameters():
                print(
                    "{}: size{}, abs_avg: {}".format(
                        name, param.size(), torch.abs(param.data.cpu()).mean()
                    )
                )
            print("################# model params ###################\n\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data-dir", type=str, metavar="PATH", default=osp.join(working_dir, "data")
    )
    parser.add_argument(
        "--logs-dir", type=str, metavar="PATH", default=osp.join(working_dir, "logs")
    )
