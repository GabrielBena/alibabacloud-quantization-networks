...
class QuaOp(object):
    ...
    def quantizeConvParams(self, T, alpha, beta, init, train_phase):
        ...
        for index in range(self.num_of_params):
            if init:
                beta[index].data = torch.Tensor(
                    [self.threshold / self.target_modules[index].data.abs().max()]
                ).cuda()
    ...
    def updateQuaGradWeight(self, T, alpha, beta, init):
        ...
        for index in range(self.num_of_params):
            ...
            beta_grad[index] = (
                y_grad.mul(self.target_modules[index].data)
                .mul(alpha[index].data)
                .mul(self.target_modules[index].grad.data)
                .sum()
            )
            alpha_grad[index] = (
                self.forward(x, T, self.QW_biases[index])
                .mul(self.target_modules[index].grad.data)
                .sum()
            )

            self.target_modules[index].grad.data = (
                y_grad.mul(beta[index].data)
                .mul(alpha[index].data)
                .mul(self.target_modules[index].grad.data)
            )
